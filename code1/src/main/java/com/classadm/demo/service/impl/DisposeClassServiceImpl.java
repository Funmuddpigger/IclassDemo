...
public class DisposeClassServiceImpl implements DisposeClassService {
    ...
    //随机分配
    public String randomClass(String class_id, int user_id, String user_name, Timestamp apply_goTime,Timestamp apply_backTime, String class_type, String class_note, String class_property,int howWeek,String severalWeeks){
        List<ClassAll> lookClassList = disposeClassMapper.lookClass("可用",class_type);
    ...
    //分配
     public  String  distributionClass(String class_id, int user_id, String user_name, Date class_goTime, Date class_backTime,String class_note, int number_people,int howWeek,String severalWeeks){
         ...
         List<Matching> matchingList = disposeClassMapper.gainMatching();
         classAll = classMessageList.get(0);
         String class_property = classAll.getClassProperty();
         String class_size = "小";
         for(int i=0;i<=matchingList.size();i++){
             if(number_people>matchingList.get(i).getClassMinCapacity()&&number_people<=matchingList.get(i).getClassMaxCapacity()){
                 class_size=matchingList.get(i).getClassSize();
                 break;
             }
         }
         if (classAll.getClassStatus().equals("可用")) {
             boolean a=judgeTime(class_id,apply_goTime,apply_backTime,howWeek,severalWeeks);
             if (judgeTime(class_id,apply_goTime,apply_backTime,howWeek,severalWeeks)) {
                 ...
                 String result=randomClass(class_id,user_id,user_name,apply_goTime,apply_backTime,class_size,class_note,class_property,howWeek,severalWeeks);
